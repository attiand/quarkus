package io.quarkus.jdbc.derby.embedded.deployment;
...
import io.quarkus.jdbc.derby.embedded.runtime.DerbyAgroalConnectionConfigurer;

public class JDBCDerbyEmbeddedProcessor {

	@BuildStep
	FeatureBuildItem feature() {
		return new FeatureBuildItem(Feature.JDBC_DERBY_EMBEDDED);
	}

	@BuildStep
	void registerDriver(BuildProducer<JdbcDriverBuildItem> jdbcDriver, SslNativeConfigBuildItem sslNativeConfigBuildItem) {
		jdbcDriver.produce(new JdbcDriverBuildItem(DatabaseKind.DERBY, "org.apache.derby.jdbc.EmbeddedDriver"));
	}
	...
	void registerDriverForReflection(BuildProducer<ReflectiveClassBuildItem> reflectiveClass) {
		...
		reflectiveClass.produce(new ReflectiveClassBuildItem(false, false, org.apache.derby.jdbc.EmbeddedDriver.class.getName()));
	}
